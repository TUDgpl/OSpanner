...
# to generate point set,  run generate.py <width> <height> <model> [parameters]

# width : width of plane
# height: height of plane
# model
#    1D uniform integer--UNIFORM1D <seed> <size> <output>
...
class InputParser( argparse.ArgumentParser ):
    ...
    def __init__(self):
        ...
        self.add_argument( 'model', metavar='model', type=lambda model: tools.Model[model],
                           choices=list( tools.Model ), nargs=1,
                           help='generator model: UNIFORM1D, ...' )
        self.add_argument( 'dimension', metavar='dimension', type=int, nargs=1,
                           help='dimension of the instance' )
        self.add_argument( 'seed', metavar='seed', type=int, nargs=1,
                           help='seed used in random number generator' )
        self.add_argument( 'point_size', metavar='point_size', type=int, nargs='1',
...
def generate_command_line():
    ...
    if args.model[0] == tools.Model.UNIFORM1D:
        Generator = generator.UniformGenerator( args )
    Generator.output()
